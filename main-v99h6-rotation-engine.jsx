              ...
              <div className="invasionRotationEngine">
                ...
                <div className="rotationNoteBox">
                  {invasionFormat==='lives'
                    ...
                    :'Use at timed rotation points. Team and player points stay attached to the current scores.'}
                </div>
              </div>
            )}
      ...
      {projectionTab==='competition'&&<>
        ...
        {competitionProjection&&<>
          ...
          {competitionProjection.mode==='invasion'&&competitionProjection.invasionTeams&&competitionProjection.invasionTeams.length>0&&<div className="projectionSection">
            ...
            <div className="projectionText">
              {competitionProjection.invasionTeams.map(team=>`Court ${team.court}${competitionProjection.activeInvasionCourt===team.court?' ★':''}: ${team.name} · ${team.players&&team.players.length?team.players.join(', '):'Waiting'} · ${competitionProjection.invasionFormat==='points'?'Points: '+(competitionProjection.invasionTeamPoints?.[team.id]||0):'Lives format'}`).join(' · ')}
            </div>
          </div>}
...
return <div>
<header className="hero"><button className="homeBtn" onClick={()=>setScreen('home')}>HOME</button><div><div className="eyebrow">CHECKERBOARD COACH</div><h1>Rebuilt Master v99h6</h1><p>Sessions · Games · Players · Competition</p></div></header>
