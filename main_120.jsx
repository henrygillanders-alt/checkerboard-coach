...
function CompetitionPlayerDisplayCard({competition}){
  ...
  return <div className="playerDisplayShell competitionPlayerDisplayShell compactCompetitionDisplay">
    ...
    {mode==='monrad'&&competition.monradRounds&&competition.monradRounds.length>0&&(()=>{
      const activeIdx=monradActiveRoundIndex();
      const round=competition.monradRounds[activeIdx]||[];
      const counts=monradCompletedCount();
      const standings=monradStats().slice(0,8);
      return <div className="competitionDisplayLayout monradPlayerCompactBoard">
        <section className="competitionDisplayHero">
          <h2>MONRAD ROUND {activeIdx+1}</h2>
          <strong>{counts.done} / {counts.total} matches complete</strong>
          <p>{round.length} matches in current round</p>
        </section>
        ...
        <section className="competitionDisplayNext">
          <h2>STANDINGS</h2>
          {standings.map((row,idx)=><p key={row.name}><strong>{idx+1}. {row.name}</strong> · {row.wins}W / {row.played}P</p>)}
        </section>
      </div>;
    })()}
