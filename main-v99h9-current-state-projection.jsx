            ...
            {invasionTeams.length>0&&(
              <div className="invasionRotationEngine">
                ...
                <div className="rotationNoteBox">
                  {invasionFormat==='lives'
                    ?'Use when a team life bank reaches 0 or at agreed rotation point. Next invader starts with current team life bank.'
                    :'Rotate at timed intervals or agreed score checkpoints. Player and team totals stay visible.'}
                </div>
              </div>
            )}
                    ...
                    {invasionTeams.map(team=>(
                      <div className={activeInvasionCourt===team.court?'invasionCourtCard activeCourtCard':'invasionCourtCard'} key={team.id} onClick={()=>setActiveInvasionCourt(team.court)}>
                        ...
                        <div className="courtCardScore">
                          {invasionFormat==='points'
                            ?<strong>{invasionTeamPoints[team.id]||0} pts</strong>
                            :<strong>{invasionTeamLives[team.id] ?? invasionStartingLives} lives</strong>}
                        </div>
        ...
        {competitionProjection&&<>
          ...
          {competitionProjection.mode==='invasion'&&<div className="projectionSection invasionCurrentStateProjection">
            ...
            <div className="currentStateHero">
              ...
              <span>Rotation {competitionProjection.invasionRotationStep||0} · Active Court {competitionProjection.activeInvasionCourt||1}</span>
            </div>
            ...
            <div className="nextActionStrip">
              {competitionProjection.invasionFormat==='lives'
                ?`Next action: update the relevant team life bank. If bank reaches 0, rotate courts. ${competitionProjection.invasionEliminated?`Trigger: ${competitionProjection.invasionEliminated}`:''}`
                :`Next action: add player score. Rotate at timed interval or agreed score checkpoint.`}
            </div>
          </div>}
...
return <div>
<header className="hero"><button className="homeBtn" onClick={()=>setScreen('home')}>HOME</button><div><div className="eyebrow">CHECKERBOARD COACH</div><h1>Rebuilt Master v99h9</h1><p>Sessions · Games · Players · Competition</p></div></header>
