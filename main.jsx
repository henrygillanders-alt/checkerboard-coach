...
function CompetitionPlayerDisplayCard({competition}){
  if(!competition){
    return <div className="playerDisplayShell competitionOnlyDisplay"><div className="playerDisplayTop"><span>COMPETITION DISPLAY</span><h1>No competition loaded</h1></div></div>;
  }
  ...
  function monradPlaceKey(roundIndex,groupId,matchIndex){return `place-${roundIndex}-${groupId}-${matchIndex}`;}
  function monradWinner(roundIndex,matchIndex,match){
    if(!match)return '';
    if(isByeName(match.b))return match.a;
    if(isByeName(match.a))return match.b;
  ...
  function monradPlacingWinner(roundIndex,group,matchIndex,match){
    ...
    if(isByeName(match.a))return match.b;
    if(isByeName(match.b))return match.a;
    return competition.monradPlacingResults?.[monradPlaceKey(roundIndex,group.id,matchIndex)]||'';
  }
  ...
  function monradCompletedCount(){
    let done=0,total=0;
    (competition.monradRounds||[]).forEach((round,ridx)=>round.forEach((match,midx)=>{total++; if(monradWinner(ridx,midx,match))done++;}));
    (competition.monradPlacingRounds||[]).forEach((round,ridx)=>round.forEach(group=>(group.matches||[]).forEach((match,midx)=>{total++; if(monradPlacingWinner(ridx,group,midx,match))done++;})));
    return {done,total};
  }
  function parseRange(range){const [a,b]=String(range||'').split('-').map(n=>Number(n));return {start:a||1,end:b||a||1};}
  function monradLivePlacings(){
    const rows={};
    (competition.playerNames||[]).forEach(name=>{rows[name]={name,place:'—',status:'Playing'};});
    Object.entries(competition.monradFinalPlaces||{}).forEach(([name,place])=>{rows[name]={name,place,status:'Settled'};});
    (competition.monradPlacingRounds||[]).forEach((round,ridx)=>round.forEach(group=>{
      const {start,end}=parseRange(group.range);
      if((group.players||[]).length===1){const name=(group.players||[]).find(p=>!isByeName(p)); if(name)rows[name]={name,place:start,status:'Settled'};}
      if((group.players||[]).length===2){
        const match=(group.matches||[])[0]; const winner=monradPlacingWinner(ridx,group,0,match);
        if(winner&&match){const loser=winner===match.a?match.b:match.a; if(!isByeName(winner))rows[winner]={name:winner,place:start,status:'Settled'}; if(!isByeName(loser))rows[loser]={name:loser,place:end,status:'Settled'};}
      }
    }));
  ...
  function roundRobinRows(){
    const rows={};
    playerNames.forEach(name=>rows[name]={name,wins:0,losses:0,played:0});
    Object.entries(competition.rrResults||{}).forEach(([key,winner])=>{
      const m=key.match(/^rr-(\d+)-(\d+)/); if(!m||!winner)return;
      const match=competition.rrFixtures?.[Number(m[1])]?.[Number(m[2])]; if(!match)return;
      [match.a,match.b].forEach(n=>{if(n&&!rows[n])rows[n]={name:n,wins:0,losses:0,played:0};});
      rows[match.a].played++; rows[match.b].played++;
      rows[winner].wins++;
      const loser=winner===match.a?match.b:match.a; rows[loser].losses++;
    });
    return Object.values(rows).sort((a,b)=>b.wins-a.wins||a.losses-b.losses||String(a.name).localeCompare(String(b.name)));
  }
  function nextRoundRobinRound(){
    const rounds=competition.rrFixtures||[];
    const idx=rounds.findIndex((round,ridx)=>round.some((_,midx)=>!competition.rrResults?.[`rr-${ridx}-${midx}`]));
  ...
  const headerSummary=mode==='matchplay'
    ? `${matchA} ${matchSummary().winsA} - ${matchSummary().winsB} ${matchB}`
    : mode==='monrad'
      ? `Monrad · ${monradCompletedCount().done}/${monradCompletedCount().total} matches complete`
      : mode==='roundRobin'
        ? `Round Robin · Round ${nextRoundRobinRound()+1}`
        : mode==='invasion'
          ? `Invasion · Rotation ${competition.invasionRotationStep||1}`
          : mode==='nsl'
  ...
  return <div className="playerDisplayShell competitionOnlyDisplay compactCompetitionDisplay">
    <div className="playerDisplayTop competitionOnlyTop">
      <span>COMPETITION DISPLAY</span>
      <h1>{title}</h1>
      <p className="competitionPlayerSummary">{headerSummary}</p>
    </div>

    {mode==='matchplay'&&(()=>{const ms=matchSummary(); const waiting=playerNames.filter(n=>n!==matchA&&n!==matchB); const next=ms.winner&&waiting[0]?`${ms.winner} vs ${waiting[0]}`:'Awaiting result'; return <div className="competitionDisplayLayout competitionOnlyGrid">
      <section className="competitionDisplayHero"><h2>CURRENT MATCH</h2><strong>{matchA} <em>vs</em> {matchB}</strong><p>{matchA} {ms.winsA} — {ms.winsB} {matchB}</p><small>{ms.scores}</small></section>
      <section className="competitionDisplayNext"><h2>NEXT UP</h2><strong>{next}</strong>{waiting.length>1&&<p>Waiting: {waiting.slice(1).join(' · ')}</p>}</section>
      <section className="competitionDisplayDraw"><h2>RESULTS</h2><ol><li>{ms.winner?`${ms.winner} def ${ms.winner===matchA?matchB:matchA} ${ms.winsA}-${ms.winsB}`:`${matchA} vs ${matchB} in progress`}</li>{waiting[0]&&<li>Next: {(ms.winner||'Winner')} vs {waiting[0]}</li>}</ol></section>
    </div>;})()}
    ...
    {mode==='monrad'&&(()=>{
      ...
      const livePlacings=monradLivePlacings().slice(0,12);
      const courtRows=hasPlacing?round.flatMap(group=>(group.matches||[]).map((match,midx)=>{const winner=monradPlacingWinner(activeIdx,group,midx,match); return {label:`Places ${group.range}`,match:`${isByeName(match.a)?'BYE':match.a} v ${isByeName(match.b)?'BYE':match.b}`,status:winner?`${winner} won`:'Next / in progress',done:!!winner};})) : round.map((match,midx)=>{const winner=monradWinner(activeIdx,midx,match); return {label:`Court ${midx+1}`,match:`${match.a} v ${isByeName(match.b)?'BYE':match.b}`,status:winner?`${winner} won`:'Next / in progress',done:!!winner};});
    ...
    {mode==='roundRobin'&&(()=>{const ridx=nextRoundRobinRound(); const round=(competition.rrFixtures||[])[ridx]||[]; const table=roundRobinRows().slice(0,10); return <div className="competitionDisplayLayout competitionOnlyGrid">
      <section className="competitionDisplayHero"><h2>ROUND {ridx+1}</h2><strong>{round.length} matches</strong><p>Round Robin court board</p></section>
      <section className="competitionDisplayDraw"><h2>COURT BOARD</h2>{round.map((match,midx)=>{const winner=competition.rrResults?.[`rr-${ridx}-${midx}`]; return <div key={midx} className={winner?'compactCourtRow completeCourtRow':'compactCourtRow'}><b>Court {midx+1}</b><span>{match.a} v {match.b}</span><em>{winner?`${winner} won`:'Next / in progress'}</em></div>;})}</section>
      <section className="competitionDisplayNext"><h2>STANDINGS</h2>{table.map((row,idx)=><p key={row.name}><strong>{idx+1}. {row.name}</strong> · {row.wins}W {row.losses}L</p>)}</section>
    </div>;})()}

    {mode==='invasion'&&<div className="competitionDisplayLayout competitionOnlyGrid"><section className="competitionDisplayHero"><h2>INVASION</h2><strong>Rotation {competition.invasionRotationStep||1}</strong><p>{competition.invasionFormat==='lives'?'Lives Format':'Points Format'}</p></section><section className="competitionDisplayDraw"><h2>COURTS</h2>{(competition.invasionTeams||[]).map((team,idx)=><div key={team.id||idx} className="compactCourtRow"><b>Court {team.court||idx+1}</b><span>{team.name}: {(team.players||[]).join(', ')}</span><em>{competition.invasionFormat==='lives'?`${invasionTeamStartLives(team)} lives`:`${competition.invasionTeamPoints?.[team.id]||0} pts`}</em></div>)}</section></div>}

    {mode==='nsl'&&<div className="competitionDisplayLayout competitionOnlyGrid"><section className="competitionDisplayHero"><h2>NSSL</h2><strong>Period {competition.nslActivePeriod||1}</strong><p>{competition.nslRoundSeconds||0}s remaining</p></section><section className="competitionDisplayDraw"><h2>TEAMS</h2><div className="compactCourtRow"><b>Teams</b><span>{competition.nslTeams} teams · {competition.nslPlayersPerTeam} players/team</span><em>Live sheet</em></div></section></div>}
  </div>;
}
...
function ShotsModule({setScreen}){
  ...
  const workingLength=[
    ...
    ['Example A','P1 hits a length. P2 arrives comfortably, maintains T position, volleys easily and attacks. Result: poor Working Length, even if the ball landed in a traditional target area.'],
    ['Example B','P1 hits a length. P2 arrives late, cannot volley and is forced to defend. Result: good Working Length, even if the ball did not land in a textbook target area.'],
  ...
  return <div className="page shotsPage shotsFoundationPage">
    ...
    {section==='increase'&&<div className="shotDetailPanel">
      <section className="shotPageBlock penetratingDrivePage">
        ...
        <div className="shotGridTwo">
          <div className="shotInfoCard exampleCard"><h3>Example A</h3><p>P1 hits a drive.</p><p>P2 arrives comfortably, maintains T position and volleys aggressively.</p><p><strong>Result:</strong> Poor Penetrating Drive, even if the ball looked technically accurate.</p></div>
          <div className="shotInfoCard exampleCard"><h3>Example B</h3><p>P1 hits a drive.</p><p>P2 arrives late, cannot volley and produces a defensive return.</p><p><strong>Result:</strong> Good Penetrating Drive, even if it was not textbook perfect.</p></div>
        </div>
      ...
      <div className="rldHero">
        ...
        <div className="cpAttributionRow">
          <div className="cpAttributionCard">
            ...
            <p>Learning is maximised when task difficulty is matched to player skill so that the learner receives the greatest amount of meaningful information without becoming overwhelmed.</p>
          </div>
          <div className="cpAttributionCard">
...
function perceptionPlaceholderCards(section){
  const all={
    ...
    interception:[
      {id:'per-in1',code:'IN1',module:'Interception',phase:'Early Take',title:'Early Take',category:'Perception',duration:8,format:'Live Rally',rld:3,gate:'Before ball drops below comfortable contact',task:'P1 reads early and takes the ball earlier in the court when available. The task is not to force attack but to gain time.',scoring:'Early balanced take = bonus. Rushed or forced take = no bonus.',rationale:'Shows whether information pickup creates useful time, not just survival.',coach:'Reward balanced early contact, not reckless rushing.',playerFocus:'Use the read to gain time.',layers:['Early Take','Opponent Information']},
      {id:'per-in2',code:'IN2',module:'Interception',phase:'Volley',title:'Volley Recognition',category:'Perception',duration:8,format:'T Rally',rld:3,gate:'Ball passing T',task:'P1 starts around the T. P2 plays with straight/cross options. P1 recognises volley affordances and intercepts when the ball is available.',scoring:'Appropriate volley = 1. Missed clear volley chance or reckless hunt recorded.',rationale:'Trains affordance detection: when the ball invites interception and when it does not.',coach:'Do not turn this into volley hunting. The player must read the affordance.',playerFocus:'Volley when the game invites it.',layers:['Volley Finish','Interception']},
      {id:'per-in3',code:'IN3',module:'Interception',phase:'Short Line',title:'Short-Line Interception',category:'Perception',duration:8,format:'Conditioned Rally',rld:3,gate:'Short line',task:'P1 scores a bonus for taking the ball before it crosses or drops behind the short-line reference when the read allows.',scoring:'Ball taken in front of short line = bonus. Wrong-foot/no-commit may also be tracked.',rationale:'The short line gives a painted, objective marker for whether the read bought court time.',coach:'Use the line as the external cue. Avoid subjective “early enough” calls.',playerFocus:'Take it before the line when it is on.',layers:['Early Take','Court Position']},
      {id:'per-in4',code:'IN4',module:'Interception',phase:'Front Court',title:'Front Court Pickup',category:'Perception',duration:8,format:'Front Court Read',rld:4,gate:'P2 contact',task:'P2 plays front-court options. P1 reads preparation, moves early and arrives balanced enough to play upward or counter-attack.',scoring:'Balanced front-court contact = 1. Late emergency scoop = 0.',rationale:'Front-court interception depends on reading before the ball has died.',coach:'Look for early organisation and spacing, not a spectacular recovery.',playerFocus:'See it before it dies.',layers:['Front Court','Tracking']},
      {id:'per-in5',code:'IN5',module:'Interception',phase:'T Intercept',title:'T Intercept',category:'Perception',duration:8,format:'Central Intercept',rld:4,gate:'Ball through central space',task:'P1 protects the T and intercepts balls travelling through central space when early information makes it possible.',scoring:'T interception = 1. Wrong-foot/no-commit recorded.',rationale:'This is the central squash affordance: own the middle by reading early.',coach:'Keep balance. No lunging across the T unless the cue is reliable.',playerFocus:'Own the middle by seeing earlier.',layers:['Opponent Off T','Volley Finish']},
      {id:'per-in6',code:'IN6',module:'Interception',phase:'Attack',title:'Volley Attack',category:'Perception',duration:8,format:'Conditioned Matchplay',rld:5,gate:'Natural',task:'P1 uses early pickup to volley with purpose: hold T, increase pressure, or finish when the option is genuine.',scoring:'Volley that creates pressure = bonus. Reckless volley error = no bonus.',rationale:'Connects interception to tactical advantage.',coach:'The volley should emerge from perception, not from a pre-decided hunt.',playerFocus:'See it early, use it well.',layers:['Volley Finish','Clean Winner']}
    ],
    deception:[
      {id:'per-dc1',code:'DC1',module:'Deception & Cue Reading',phase:'Racquet',title:'Racquet Face Reading',category:'Perception',duration:7,format:'Front / Midcourt Read',rld:3,gate:'P2 contact',task:'P2 plays options where racquet face provides useful but incomplete information. P1 reads face, organises, then confirms with early ball flight.',scoring:'Correct early commitment = 1. Wrong-foot/no-commit recorded.',rationale:'Develops sensitivity to racquet information while preventing blind guessing.',coach:'Use questions sparingly: “what did the racket show?”',playerFocus:'See the face, then confirm.',layers:['Opponent Information','Quiet Eye']},
      {id:'per-dc2',code:'DC2',module:'Deception & Cue Reading',phase:'Body',title:'Shoulder & Hip Reading',category:'Perception',duration:7,format:'Back Corner Read',rld:3,gate:'P2 preparation',task:'P2 plays from rear-court options. P1 looks for shoulder, hip and spacing information before the ball leaves the strings.',scoring:'Correct read before bounce/front wall = 1. Wrong-foot/no-commit recorded.',rationale:'Broadens cue search beyond the ball and racket.',coach:'Keep cues external and functional: “what did their shape allow?”',playerFocus:'Read the shape, not just the ball.',layers:['Opponent Information','Early Cue Search']},
      {id:'per-dc3',code:'DC3',module:'Deception & Cue Reading',phase:'Hold',title:'Hold Recognition',category:'Perception',duration:8,format:'Live Choice',rld:4,gate:'P2 release',task:'P2 is allowed to hold the ball. P1 must delay commitment until the cue becomes reliable, then move decisively.',scoring:'Correct delayed commitment = 1. Buying the hold/wrong-foot = 0.',rationale:'Teaches that earlier is not always better. The best read is the earliest reliable read.',coach:'Praise waiting when the cue is genuinely unreadable. This stops blind early guessing.',playerFocus:'Do not buy the hold.',layers:['Disguise','Opponent Information']},
      {id:'per-dc4',code:'DC4',module:'Deception & Cue Reading',phase:'Disguise',title:'Disguise Recognition',category:'Perception',duration:8,format:'Live Rally',rld:4,gate:'Reliable cue',task:'P2 can show one shot and play another. P1 searches for information that still specifies where the ball is going.',scoring:'Clean read, wrong-foot and no-commit counts. Optional note: which cue failed.',rationale:'Develops calibration between reliable and unreliable information.',coach:'The aim is not to never be fooled. The aim is to learn which cues survive disguise.',playerFocus:'Find the cue that stays true.',layers:['Disguise','Early Cue Search']},
      {id:'per-dc5',code:'DC5',module:'Deception & Cue Reading',phase:'Matchplay',title:'Deception Matchplay',category:'Perception',duration:10,format:'Conditioned Matchplay',rld:5,gate:'Natural',task:'Normal or conditioned game where P2 may use holds and disguise. P1 plays normally while tracking wrong-foots and successful waits.',scoring:'Normal scoring plus wrong-foot count and successful read count.',rationale:'Tests whether cue reading transfers when deception has tactical consequence.',coach:'Do not overcoach after every deception. Let patterns emerge across the block.',playerFocus:'Play the ball, not the fake.',layers:['Disguise','Matchplay']}
    ]
  };
  return all[section]||[];
}
...
function PerceptionModule({setScreen,setSession,onAddToSession,embedded=false}){
  ...
  const activeLayers=Array.isArray(activeConfig.layers)?activeConfig.layers:[];
  ...
  function configuredPerceptionGame(game){
    ...
    // Perception cards may carry suggested layers in their data, but these must never become active by default.
    // Only coach-selected layers from cfg.layers are allowed to reach Player View, Session Builder, scoring, or projection.
    delete base.layers;
    ...
    <div className="perceptionWorkArea">
      ...
      {active&&<div className="perceptionDetail gameCard universalPerceptionGameCard">
        ...
        <CollapsibleLayer num="1" title="Game Logic" subtitle="What counts — eligibility and validity" color="green" defaultOpen={true}>
          <div className="perceptionLogicSummary">
            <div className="infoBox"><strong>Valid Attempt</strong><p>{activeCustom.task||active.task}</p></div>
            <div className="infoBox"><strong>Eligibility Gate</strong><p>{activeCustom.gate||active.gate||'Coach-defined gate'}</p></div>
            <div className="infoBox"><strong>Invalid / No Score</strong><p>{activeCustom.invalidRule||'Late action, wrong-foot, no-commit, pre-lean or forced action outside the task rule does not count unless the coach has set a custom consequence.'}</p></div>
          </div>
          <div className="atbOverlayChips perceptionLogicChips">
            {['Valid Attempt','Eligibility Gate','No Pre-Lean','Wrong-Foot','No-Commit','Early Directional Commitment','Recover–Organise–Commit','T Intercept'].map(layer=><button key={layer} type="button" className={activeLayers.includes(layer)?'atbOverlayActive':'atbOverlayChip'} onClick={()=>togglePerceptionLayer(active,layer)}>{activeLayers.includes(layer)?'✓ ':'+ '}{layer}</button>)}
          </div>
          <details className="perceptionCustomOverride"><summary>Custom text override</summary>
        ...
        <CollapsibleLayer num="2" title="Scoring Logic" subtitle="How points are awarded" color="gold" defaultOpen={true}>
          ...
          <details className="perceptionCustomOverride"><summary>Custom scoring text</summary>
            <div className="perceptionLogicEditor">
    ...
    {activeModule==='tau'&&<div className="l0ModuleSection">
      ...
      {tauTab==='science'&&<div className="tauSciencePanel">
        ...
        <div className="tauScienceHero">
          <h3>What Is Tau?</h3>
          <p>Tau (τ) is an optic variable first described by David Lee in 1976. It refers to information available in the visual array that specifies <em>time to contact</em> — how long until a moving object arrives at the player.</p>
...
function CustomGameBuilder({onAddToSession}){
  ...
  function generateRandom(){
    ...
    setRandomResult(randomMode==='Blind'?'Blind random conditions generated. Coach reveals conditions when appropriate.':'P1: '+a+' · P2: '+b);
  }
  ...
  return <div className="atbBuilder">
    ...
    {activeTab==='court'&&<div className="atbCourtPanel">
      ...
      <div className="atbCourtDiagram">
        ...
        <div className="atbIntegratedFloor">
          <div className="atbWallLabel">Floor Zones — 1 to 4</div>
          <div className="atbFloorGrid">
            ...
            <div className="atbFloorZone" style={{background:'#4ade8022',borderColor:'#4ade80'}}><span>4</span><small>Back Left</small></div>
            <div className="atbFloorZone" style={{background:'#ffd98022',borderColor:'#ffd980'}}><span>3</span><small>Back Right</small></div>
          </div>
        </div>
      </div>
...
function Competition({players=[],initialInvasionFormat='lives',onInvasionFormatChange=()=>{}}){
    ...
    function changeGame(gameIdx,side,value){
      ...
      setGames(prev=>{
        ...
        if(scoringMode==='normal'){
          ...
          if(side==='a'){
            // typed in P1 box = P1 is the loser, P2 wins
            next[gameIdx]={a:cleaned,b:winnerScore,loserSide:'a'};
          } else {
            // typed in P2 box = P2 is the loser, P1 wins
  ...
  },[mode,scoringMode,competitionLayers,competitionCbCode,playerBounces,manualPlayers,matchScore,matchplayMatchFormat,matchPlayers,matchScoring,competitionMatchScores,rrFixtures,rrResults,rrBoxFixtures,rrFinalFixtures,monradRounds,monradResults,monradPlacingRounds,monradPlacingResults,monradFinalPlaces,invasionFormat,invasionStartingLives,invasionTeams,invasionTeamLives,invasionTeamPoints,invasionPlayerPoints,invasionInvaderOverrides,invasionCarryLives,invasionFinishLives,invasionPlayerRound,invasionCourtRound,invasionGameStarted,invasionCourtAssignmentMode,invasionCourtAssignments,invasionRotationStep,activeInvasionCourt,invasionEliminated,nslTeams,nslPlayersPerTeam,nslPeriod1,nslPeriod2,nslPeriod3,nslOvertime,nslScores,nslActivePeriod,nslRoundSeconds,nslPowerPlayTeam,nslPowerPlaySeconds,nslPowerPlayActive]);
...
,
  ...
  const cases={
    ...
    'Emotional Reactions':{
      ...
      why:'Error recovery is trainable behaviour. Elite players regain useful attention quickly after disruption.'
    }
  };
...
/* ── VISION & PERCEPTION MODULE — v100h68 ─────────────────────────────── */
const VISION_ACTIVITIES=[
  {title:'Two-Colour Racquet',rld:'RLD 3',section:'Visual Search',purpose:'Guide gaze back toward the opponent at or before contact.',setup:'Use a racquet with two clearly different colours or add temporary coloured tape/markers to opposite sides of the frame.',task:'During live or semi-live rallies, coach randomly asks what colour was visible when the opponent struck the ball.',rationale:'The coloured racquet is not the cue. It is an attentional guide that helps the player search in the right general area so they can begin detecting specifying information from the whole opponent-racquet scene.',fade:'Fade questioning as soon as the player naturally looks back towards the opponent. Reintroduce only if gaze regresses to ball/front-wall watching.',risk:'If overused, the colour or racquet may become the exclusive cue and narrow attention away from earlier body cues such as shoulders, trunk, preparation, racquet path and weight transfer.'},
  {title:'White Ball on White Court',rld:'RLD 4',section:'Opponent Information',purpose:'Reduce dependency on late ball flight and increase use of opponent body cues.',setup:'Use a white ball on a white or light court only where safe and appropriate. Start with controlled rallies before competitive versions.',task:'Players rally or compete while the reduced ball contrast forces earlier information pickup from the opponent.',rationale:'Less visible ball flight increases the value of shoulder rotation, racquet preparation, swing speed and contact information.',fade:'Return to normal ball once players are searching earlier and reading the opponent more reliably. Reintroduce briefly if visual search collapses back to late ball tracking.',risk:'Do not use where safety or confidence is compromised. The aim is better information pickup, not making the task artificially impossible.'},
  {title:'Quiet Eye Contact Windows',rld:'RLD 2–4',section:'Quiet Eye',purpose:'Stabilise gaze at opponent contact and own contact without freezing movement.',setup:'Use cooperative, conditioned or live rallies depending on level.',task:'Player identifies the opponent contact window, then stabilises gaze through their own contact before recovering vision back to the opponent.',rationale:'Quiet Eye is treated as functional information pickup, not staring. The gaze must support action.',fade:'Reduce verbal reminders and allow the gaze routine to self-organise inside normal play.',risk:'Avoid making players rigid. Stable eyes should improve movement freedom, not reduce it.'},
  {title:'Opponent-Racquet Scene Reading',rld:'RLD 4–5',section:'Anticipation',purpose:'Expand attention from one cue to the full movement scene.',setup:'Use rallies where opponent can vary drive, drop, boast, cross-court or hold.',task:'Between points, ask what information specified the shot: shoulder, trunk, racquet face, racquet path, swing amplitude, timing or body position.',rationale:'The target behaviour is not cue guessing. It is perception of specifying information from the whole opponent-racquet system.',fade:'Move from coach questioning to player self-report and then to normal play.',risk:'Too many questions can make players analytical. Use intermittent, short debriefs.'}
];
function VisionPerceptionModule({setScreen}){
  const sections=['Visual Search','Opponent Information','Quiet Eye','Anticipation'];
  return <div className="page visionPage">
    <div className="pageTop"><div><h1>Vision & Perception</h1><p className="mutedText">Opponent information pickup · visual search · anticipation · constraint fading</p></div><button className="secondaryBtn" onClick={()=>setScreen('home')}>Home</button></div>
    <div className="visionHero"><strong>See the opponent-racquet scene, not just the ball.</strong><span>The objective is better information pickup in representative squash environments.</span></div>
...
const BTS_GAMES=[
  ...
  {id:'difference',group:'Tier 3',title:'Blind Difference',tier:'Tier 3',level:'Level 4+',format:'1 v 1',deck:null,mechanism:'Identity uncertainty',suit:'identity',arb:'App',pairs:[[4,7],[4,8],[5,9],[3,6],[5,8]],blurb:'A known pair of targets, dealt blind. You hold one — but not which.',setup:'App-required. Pair selected at random with minimum gap 3. Players are told the pair, not their own target.',steps:['The app selects a target pair, for example 4 and 8.','Players are told the pair.','P1 secretly receives one target; P2 the other.','Neither player knows which target they hold.','Play normal rally scoring; app tracks both targets.','App ends the game when the correct hidden target is reached.','Reveal at end.'],scoring:'Rally winner +1. App ends game when a player reaches their unknown target.',coachMsg:'Can you compete without knowing whether you are favourite or underdog?',obs:['Tolerance of total uncertainty','Behaviour at the Certainty Cliff','Who plays the same regardless of standing']},
